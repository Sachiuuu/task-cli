...
**Git commits produced:** 7. `Renumber task IDs sequentially after deletion` 8. `Add chalk, log-symbols, cli-table3, prompts for improved UX`
...
**Git commits produced:** 9. `Add undo feature with snapshot-based state restore` 10. `Add undo field to data model with backward-compat loading` 11. `Add stats, undo command, aliases, and screen clear on each command`
...
**Git commits produced:** 13. `Rename binary alias from 't' to 'tsk' to avoid terminal conflicts` 14. `Add confirmations, colored stats with emojis, aliases in help`
...
**Git commits produced:** 16. `Add todo command, due dates, tags, and parseDueDate` 17. `Add todo command, --due/--tag options, tag grouping in list`
...
**Git commits produced:** 18. `Replace global nextId with per-tag tagCounters for independent ID sequences` 19. `Add update-due command, --tag option on all ID commands, separate tables per tag`

---

## Prompt 7 (Priority, Sort, Clear-all, Tag targeting fix, Help update)

**Prompt:**

> Me gustaria agregar un "Priority" feature a las task q el usuario desee al momento de agregar un nuevo task, y que estas se puedan filtrar por un comando llendo de prioridad alta a baja y viceversa por cada tabla. Tambien para q el col de "Due" no se vea vacio, cada task q no tenga asignado un fecha coloques en la casilla "none" y q tambien se pueda filtrar por la fecha mas cercana tomando el dia actual y haciendolo por cada table aparte. Por ultimo agrega un feauture llamado "delete all" o "clear all" el cual eliminara todas las tasks para comenzar desde 0. Actualiza debidamente el readme.md incluyendo los nuevos features/commands para q el usuario tenga mejor entendimiento de la herramineta.al tratar de elminar un tsk de id 1 se deberia de eliminar el q no tiene tag, osea el q esta en el "other" table. los tasks q esten con un tag para q sean eliminados si o si el commando tiene q especificar el tag del elemento. agrega esto en el readme file y tmb haz un update al help window del cli por si el usuario no se acuerda sobre los comandos de update, sort, tag, etc.

**What was done:**

- Added `priority: "high" | "medium" | "low" | null` field to tasks — set via `--priority` (or `-p`) on `add`
- `src/task.js`: updated `addTask()` to accept and validate `priority`, added `clearAll()` function
- `index.js`: added `priorityCell()` helper (🔴/🟡/🔵/—), `sortTasks()` helper, Priority column in all tag tables, `--sort priority|priority-asc|due|due-desc` option on `list`, `--priority` option on `add`, `clear` command (alias `clr`)
- Due column now shows `none` (dimmed) instead of blank for tasks without a due date
- Fixed `findTask()` targeting rule — without `--tag`: targets untagged (Other) tasks only; tagged tasks always require `--tag`. Error message guides the user with the correct syntax
- Expanded `addHelpText` to include key options reference, sort/filter options, and tag targeting rules
- Updated README with tag targeting rules table and updated shortcuts
- 117 tests passing

**Git commits produced:** 20. `Add priority field, --sort option, Due 'none' display, and clear command` 21. `Fix tag targeting: untagged by default, tagged always require --tag; expand help`
